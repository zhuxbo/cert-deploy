...
```json
{
  ...
  "errors": { "error_code": "rate_limited", "retry_after": 42 }
}
```

| `error_code`       | 含义                                           | 客户端应对               |
| ------------------ | ---------------------------------------------- | ------------------------ |
| `rate_limited`     | 触发限流；带 `retry_after`（当前窗口剩余秒数） | 本轮停止，不在窗口内重试 |
...
| `order_not_found`  | 订单不存在或不在本 token 可见范围              | 停止，等人工核对配置     |
| `cert_not_found`   | 订单存在但无可用证书                           | 停止，等人工核对配置     |
...
**不分页**：空参数时固定返回最新 100 条 active 订单，`page` / `page_size` 请求参数会被忽略
（越界值也不报错），`total` 即本次实际返回条数、不是全量总数；逗号批量本身上限 100，一次即取完。

`order` 非空的按 ID / 按域名查询仍走原分页封装、仍接受 `page` / `page_size`，其 `total` 是匹配
总数——单域名匹配超过 100 个订单时 `total` 会大于 `data` 长度。客户端一律单次请求取完即止，
不得按 `total` 翻页——翻页循环没有页数与条数上限，终止只依赖服务端自报的 `total` 与非空页，
两者同时失真即无限翻页且累积内存无界。

空参数响应保留 `page` / `page_size` 字段（固定 1 / 100）以稳定结构。
...
- 时限判定要求时钟单调：时间戳缺失/损坏、时钟回拨、间隔大到不可信（超过 60 天）时重新锚定当前时间而非判定停更，方向保守。
...
| 批量查询上限       | 100     | 逗号分隔项数上限，同时是空参数查询的固定返回条数；不分页（见 2.3）                                                                                |
