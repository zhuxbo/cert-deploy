...
`retry_after` 是**当前限流窗口的剩余秒数**，**不是**「这么久之后一定可重试」。服务端用的是
滑动窗口加权（`估算值 = 当前窗口计数 + 上一窗口计数 × 上一窗口剩余权重`）：睡满 `retry_after`
后恰好落在新窗口起点，此时上一窗口（刚刚超限的那个）权重为 1、全额计入，估算值必然仍超限——
**按它定时重试注定再被拒一次，且那次重试又会把计数器垫高，把真正的恢复时间继续往后推**。

因此客户端**不得**把它当作 HTTP `Retry-After` 语义做定时重试。正确用法只有两种：本轮直接停止，
等下一个调度周期（分钟级以上）自然重来；或仅把它写进日志/告警文本，供运维判断「大约多久后
不再是限流状态」。
